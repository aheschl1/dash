...
| `titler.py` | **Conversation titling sidecar.** After a conversation's first user turn, `maybe_propose_title` (called after each save on both the POST and WS paths) hands off to a **daemon thread** that makes a **separate, fresh-context** LLM call (its own "pick a title" system prompt — not the agent loop, not its tool-calling prompt) to propose a ≤6-word title, persisted via `db.chat_set_auto_title`. **Non-blocking:** the turn returns its answer immediately; the title lands asynchronously and `db.chat_list` prefers `auto_title` over the first-message-derived `title`. Fires once (in-memory `_seen` guard + a DB re-check), drops empty/failed proposals (fallback title remains, no retry). Threshold via `AGENT_TITLE_AFTER_TURNS` (default 1). The frontend re-fetches the conversation list a few seconds after `done` to pick up the async title. |
...
chat_sessions — id (uuid), title (first-message fallback), auto_title (LLM titling sidecar, preferred by chat_list), turns, messages (JSONB raw message list), created_at, updated_at (persisted agent chats; most-recent-N retained)
